            ...
            <form action="{{ route('materials.store') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="reference" id="reference"
                        ...
                        value="{{ 'M-' . strtoupper(uniqid()) }}" readonly>
                </div>

                <!-- Identifiant de l'appareil -->
                <div class="mb-4">
                    <label for="device_identifier_id" class="block text-sm font-medium text-gray-700">Référence
                        produit</label>
                    <select name="device_identifier_id" id="device_identifier_id"
                        ...
                        @foreach ($deviceIdentifiers as $identifier)
                            <option value="{{ $identifier->id }}"
                                {{ old('device_identifier_id') == $identifier->id ? 'selected' : '' }}>
                                {{ $identifier->name }}
                            </option>
                        @endforeach
                    </select>
