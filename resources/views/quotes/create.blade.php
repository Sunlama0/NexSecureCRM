...
@section('content')
    <div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        <div class="bg-white shadow rounded-lg p-6">
            <h2 class="text-2xl font-bold mb-6">Créer un nouveau devis</h2>
            ...
            <div id="errorMessage" class="hidden bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-6" role="alert">
                <span class="block sm:inline">Veuillez remplir tous les champs obligatoires avant de passer à l'étape suivante.</span>
            </div>
            ...
            <form id="quoteForm" action="{{ route('quotes.store') }}" method="POST">
                ...
                <div id="step1" class="step">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="quote_number" id="quote_number" value="{{ $quoteNumber }}" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" readonly>
                    </div>

                    <!-- Date du devis -->
                    <div class="mb-4">
                        <label for="quote_date" class="block text-sm font-medium text-gray-700">Date du devis</label>
                        <input type="date" name="quote_date" id="quote_date" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                    </div>

                    <!-- Date d'expiration -->
                    <div class="mb-4">
                        <label for="expiration_date" class="block text-sm font-medium text-gray-700">Date d'expiration</label>
                        <input type="date" name="expiration_date" id="expiration_date" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                    </div>

                    <!-- Objet -->
                    <div class="mb-4">
                        <label for="subject" class="block text-sm font-medium text-gray-700">Objet</label>
                        <input type="text" name="subject" id="subject" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                    </div>

                    <!-- Bouton suivant -->
                    <div class="mt-6 flex justify-end">
                        <button type="button" id="nextStep" class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700">
                            Suivant
                        </button>
                    </div>
                </div>
                ...
                <div id="step2" class="step hidden">
                    ...
                    <table class="min-w-full table-auto mb-4">
                        ...
                        <tbody id="itemsWrapper">
                            <tr class="article-row">
                                <td class="px-4 py-2">
                                    <input type="text" name="items[0][description]" class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                                </td>
                                <td class="px-4 py-2">
                                    <input type="number" name="items[0][quantity]" min="1" class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm quantity" required>
                                </td>
                                <td class="px-4 py-2">
                                    <input type="number" step="0.01" name="items[0][rate]" min="0" class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rate" required>
                                </td>
                                <td class="px-4 py-2">
                                    <input type="number" step="0.01" name="items[0][tax]" min="0" class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm tax" required>
                                </td>
                                <td class="px-4 py-2">
                                    <input type="number" step="0.01" name="items[0][discount]" min="0" class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm discount">
                                </td>
                    ...
                    <button type="button" id="addArticle" class="mb-4 text-indigo-600 hover:text-indigo-900">Ajouter un article</button>
                    ...
                    <div class="mt-6 flex justify-between">
                        <button type="button" id="prevStep" class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700">
                            Retour
                        </button>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const requiredFields = ['client_id', 'quote_number', 'quote_date', 'expiration_date', 'subject'];
